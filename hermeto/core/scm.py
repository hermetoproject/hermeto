...
def _find_submodule_containing_path(
    repo: Repo, repo_root: Path, target_path: Path
) -> Optional[tuple[git.Submodule, Path]]:
    """Find the submodule containing the target path, if any.

    :param repo: Git repository to search in
    :param repo_root: Root path of the repository
    :param target_path: Path to find containing submodule for
    :return: Tuple of (submodule, submodule_path) or None if no submodule contains the path
    """
...
def get_repo_for_path(repo_root: Path, target_path: Path) -> tuple[Repo, Path]:
    ...
    """
    # Convert relative path to absolute for consistent processing
    ...
    # Iteratively find the deepest submodule containing the path
    while (
        result := _find_submodule_containing_path(current_repo, current_repo_root, target_path)
    ) is not None:
        submodule, submodule_path = result
        current_repo = _get_submodule_repo(submodule)
        current_repo_root = submodule_path

    # Calculate relative path from the final repository root
    relative_path = target_path.relative_to(current_repo_root)
