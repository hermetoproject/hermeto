...
class HEAD(BaseHEAD):
    ...
    def reset(self, **kwargs: Any) -> "HEAD":  # type: ignore[override]
...
class Repo(BaseRepo):
    ...
    def clone_from(  # type: ignore[override]
        ...
        """Clone repository and return Repo with error handling."""
        try:
            BaseRepo.clone_from(url, to_path, **kwargs)
            return cls(to_path)
        except GitCommandError as ex:
            log.warning(
...
def get_repo_id(repo: StrPath | Repo | BaseRepo) -> RepoID:
    ...
    elif isinstance(repo, BaseRepo) and not isinstance(repo, Repo):
...
def _get_submodule_repo(submodule: git.Submodule) -> Repo:
    ...
    try:
        ...
        return Repo(base_repo.working_dir)
...
def get_repo_for_path(repo_root: Path, target_path: Path) -> tuple[Repo, Path]:
    ...
    while (submodule := _find_submodule_containing_path(current_repo, target_path)) is not None:
        ...
        current_repo = Repo(submodule_repo.working_dir)
...
def _reset_git_head(repo: Repo, ref: str) -> None:
    try:
        ...
        log.exception(
            "Failed on checking out the Git ref %s, exception: %s, exception-msg: %s",
            ref,
            type(ex).__name__,
            str(ex),
        )
