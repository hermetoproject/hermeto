...
MaybeHttpUrl = HttpUrl | None


def _ensure_proxy_url_ends_with_a_slash(url: MaybeHttpUrl) -> MaybeHttpUrl:
...
def _proxy_url_must_not_contain_credentials(url: MaybeHttpUrl) -> MaybeHttpUrl:
    if url is None:
        return None
    if url.username or url.password:
        raise InvalidInput(
            reason="User-provided proxy URL appears to contain embedded credentials.",
            solution="Please ensure that proxy URL does not contain credentials "
...
class NpmSettings(ProxyMixin, BaseModel, extra="forbid"):
    """Npm settings."""
