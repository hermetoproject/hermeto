...
def _resolve_packages(request: Request) -> RequestOutput:
    ...
    for t in requested_types:
        if t in EXPERIMENTAL_MAPPING:
            normalized_type = EXPERIMENTAL_MAPPING[cast(ExperimentalPMInputType, t)]
            normalized_types.add(normalized_type)
            experimental_types.add(normalized_type)
        else:
            normalized_types.add(cast(PackageManagerType, t))

    if experimental_types:
        requested_types = cast(set, normalized_types)
