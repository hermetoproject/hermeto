...
def _resolve_yarn_project(
    ...
    if mode != Mode.PERMISSIVE:
        raise PackageRejected(
            ...
            solution="Consider whether permissive mode is acceptable for your use case.",
        )
...
def _parse_lockfile_git_deps(project: Project) -> list[GitDep]:
    ...
    for key, entry in lockfile_data.items():
        ...
        try:
            ...
            log.debug("Skipping lockfile entry %r: could not parse locator", key)
            continue
...
def _clone_and_resolve_git_deps(
...
) -> tuple[list[ProjectFile], dict[str, str]]:
    ...
    :return: tuple of (project_files, tarball_vcs_url_map) where tarball_vcs_url_map maps
        tarball absolute path strings to vcs_url qualifiers for PURL generation.
    ...
    tarball_vcs_url_map: dict[str, str] = {}
