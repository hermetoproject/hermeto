...
class _SourceModel(pydantic.BaseModel, extra="ignore"):
    """Fields common to remote source tables in a pylock package."""
    ...
    def _reject_local_path(cls, data: Any) -> Any:
        if isinstance(data, dict) and "path" in data:
            raise PackageRejected(
                ...
                solution="If you maintain this lockfile, use a remote URL instead of a local path.",
            )
        return data


class _VCSSourceModel(_SourceModel):
    """The ``[packages.vcs]`` table of a pylock ``[[packages]]`` entry."""
...
class _ArchiveSourceModel(_SourceModel):
    """The ``[packages.archive]`` table of a pylock ``[[packages]]`` entry."""

    url: str


class _ArtifactModel(_SourceModel):
    """Fields common to sdist and wheel entries in a pylock package."""

    name: str | None = None
    size: int | None = None


class _SdistModel(_ArtifactModel):
    """The ``[packages.sdist]`` table of a pylock ``[[packages]]`` entry."""


class _WheelModel(_ArtifactModel):
    """One ``[[packages.wheels]]`` entry of a pylock ``[[packages]]`` entry."""


class _PackageModel(pydantic.BaseModel, extra="ignore"):
    """A ``[[packages]]`` entry in a pylock lockfile."""
    ...
    sdist: _SdistModel | None = None
    wheels: list[_WheelModel] | None = None
    ...
    def _reject_directory(self) -> Self:
        if self.directory is not None:
            raise PackageRejected(
                ...
                solution="If you maintain this lockfile, use a fetchable source instead of a directory dependency.",
            )
        return self


class _LockfileModel(pydantic.BaseModel):
    """The top-level structure of a pylock lockfile."""
...
class PyLockPackage(PipRequirement, ABC):
    """A package as defined in pylock.
    ...
    def from_package_data(cls, data: dict[str, Any], path: Path) -> "PyLockPackage":
        """Build a ``PyLockPackage`` from a raw ``[[packages]]`` entry.

        Validates ``data``, dispatches to the concrete package class, and sets
        fields shared by every package kind. Kind-specific fields are parsed by
        ``_parse_source_fields`` on the concrete class.
        """
        try:
        ...
        if "vcs" in data:
            package = PyLockVCSPackage._parse_source_fields(data, path)
        elif "archive" in data:
            package = PyLockArchivePackage._parse_source_fields(data, path)
        else:
            package = PyLockIndexPackage._parse_source_fields(data, path)
    ...
    @classmethod
    @abstractmethod
    def _parse_source_fields(cls, data: dict[str, Any], path: Path) -> Self:
        """Parse kind-specific source fields from a validated ``[[packages]]`` entry.

        ``from_package_data`` sets ``name``, ``version``, and other common fields
        after dispatch.
        """


class PyLockVCSPackage(PyLockPackage):
    """A VCS package as defined in pylock."""
...
class PyLockArchivePackage(PyLockPackage):
    """An archive package as defined in pylock."""
...
class PyLockIndexPackage(PyLockPackage):
    """An index package as defined in pylock."""
    ...
    @classmethod
    def _parse_source_fields(cls, data: dict[str, Any], path: Path) -> Self:
        """Parse index, sdist, and wheel fields for a PyPI-resolved package."""
