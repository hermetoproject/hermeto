...
class RetryAfterJitterRetry(aiohttp_retry.JitterRetry):
    """Respect server-requested delays to avoid repeating rate-limited requests too soon."""
    ...
    def get_timeout(
        ...
        if parsed is None:
            log.warning(
                "Unparseable Retry-After header from %s: '%s'", response.url.host, retry_after
            )
            return default
        if parsed < 0:
            log.warning(
    ...
    def _parse_retry_after(retry_after: str) -> float | None:
        ...
        try:
            ...
            # HTTP dates are UTC, including the timezone-less asctime format (RFC 9110).
