...
def _try_git_dep(resolution: str) -> GitDep | None:
    ...
    # Validate patches with parse_locator (nested git fails; npm patches pass),
    # then skip — they are not git deps to rewrite.
    if protocol == "patch":
...
def _resolve_yarn_project(
    ...
    if workspaces:
        ...
        _strip_workspace_scripts(project.source_dir)
...
def _clone_git_deps(
    ...
    for dep in git_deps:
        ...
        host, namespace, repo, ref = (
            clone_url.host,
            clone_url.namespace,
            clone_url.repo,
            clone_url.ref,
        )
        if not host or not namespace or not repo or not ref:
            raise UnexpectedFormat(
                f"Cannot parse git URL: {dep.clone_url}",
                solution="Ensure the git dependency has a valid URL and commit ref.",
            )
        source_key = (host, namespace, repo, ref)
