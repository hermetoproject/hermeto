...
def _resolve_env_vars(build_config: BuildConfig, relative_to_path: Path) -> list[tuple[str, str]]:
    ...
    Legacy ``kind="path"`` values are joined to ``output_dir``; template
    placeholders such as ``${GOMODCACHE}`` are expanded against resolved mappings::
    ...
    ...     ],
    ... )
    >>> _resolve_env_vars(config, Path("/tmp/output"))
    [('GOMODCACHE', '/tmp/output/deps/gomod/pkg/mod'), ('GOPROXY', 'file:///tmp/output/deps/gomod/pkg/mod/cache/download')]
    """
