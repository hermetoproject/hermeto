...
_codes_to_names: dict[int, str] = dict(zip(_exit_codes.values(), _exit_codes.keys()))
...
def get_error_name_from_code(code: int) -> str | None:
    ...
    """
    return _codes_to_names.get(code)


class InternalLogicError(ValueError):
    """An error that should not happen during normal operation.

    Used to signal inconsistency in code. Raise this when reaching a branch
    that should be unreachable (e.g. after a change)."""
...
class BaseError(Exception):
    ...
    def __init_subclass__(cls) -> None:
        ...
        if class_name not in _exit_codes:
            raise InternalLogicError(f"No exit code found for {class_name}")
