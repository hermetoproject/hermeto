...
def _filter_packages_with_rust_code(
    packages: list, app_path: RootedPath, output_dir: RootedPath, source_dir: RootedPath
) -> list:
    packages_containing_rust_code = []
    ...
    for p in tar_packages:
        ...
        try:
            tf.getmember(f"{p['package']}-{p['version']}/Cargo.toml")
        ...
        packages_containing_rust_code.append(
            CargoPackageInput(type="cargo", path=str(path_to_use.path.relative_to(output_dir.path)))
        )
