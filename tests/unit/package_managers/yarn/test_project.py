...
def test_parse_invalid_package_json_file(rooted_tmp_path: RootedPath) -> None:
    with pytest.raises(InvalidLockfileFormat, match="lockfile.*format is not valid"):
...
def test_parse_empty_folder(rooted_tmp_path: RootedPath) -> None:
    message = "lockfile 'package.json' not found under"
    with pytest.raises(LockfileNotFound, match=message):
