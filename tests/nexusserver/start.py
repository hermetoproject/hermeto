...
try:
    from tests.nexusserver.repositories import (
...
DEFAULT_NEXUS_IMAGE = "sonatype/nexus3:latest"
...
class NexusClient:
    ...
    def wait_for_ready(self, timeout: int) -> None:
        ...
        status_url = f"{self.base_url}/service/rest/v1/status"
    ...
    def accept_eula(self) -> None:
        ...
        get_response = self.session.get(url, auth=self.auth)
        ...
        post_response = self.session.post(url, auth=self.auth, json=eula_data)
    ...
    def enable_anonymous_access(self) -> None:
        ...
        response = self.session.put(url, auth=self.auth, json=payload)
        response.raise_for_status()
    ...
    def create_proxy_repository(self, config: ProxyRepositoryConfig) -> None:
        ...
        response = self.session.post(url, auth=self.auth, json=config.to_api_payload())
        response.raise_for_status()
