...
class TestParameters:
    ...
    # Network mode to use when building the verification container (None => default Podman behavior)
    container_network: str | None = "none"
    # When true, mounts the rpm repo config files generated by fetch-deps into the verification container.
    # Set to False when the container build should use the base image's normal repository configuration.
    mount_rpm_repos: bool = True
...
def build_image_for_test_case(
    ...
    if mount_rpm_repos and Path(rpm_repos_path).exists():
        flags.extend(
            [
                "-v",
...
def fetch_deps_and_check_output(
    ...
    # The JSON string representing the packages is the last argument
    cmd.append(json.dumps(test_params.packages))

    # Debug: record full command so we can inspect what was passed into the container
    log.info("Container command: %s", cmd)
    ...
    if test_params.check_output:
        ...
        # The following assertion can fail when a backend produced no output;
        # capture container stdout to make debugging easier.
        if any(pkg.get("type") == "pip" for pkg in test_params.packages):
            # ensure the SBOM actually contains a pip annotation/component
            sbom_text = json.dumps(sbom_for_comparison)
            assert "hermeto:backend:pip" in sbom_text, (
                "SBOM produced by fetch-deps did not include any pip components.\n"
                f"Container output:\n{output}\n\nSBOM:\n{sbom_text}"
            )
...
def build_image_and_check_cmd(
    ...
    with build_image_for_test_case(
        ...
        mount_rpm_repos=mount_rpm_repos,
    ) as test_image:
