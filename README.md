...
* Maven is considered experimental
...
Hermeto supports maven by parsing one or more `lockfile.json` files generated by the
[maven-lockfile plugin](https://github.com/chains-project/maven-lockfile). Maintainers
are responsible for adding this plugin to their code and invoking the required Maven
targets to generate the lockfile and build using a "frozen" `pom.xml` file.
