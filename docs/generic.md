...
```js
{
  ...
  // defaults to "."
  "path": ".",
  // option to specify lockfile path: absolute or relative to "path"
  // defaults to "artifacts.lock.yaml", resolved relative to path
...
JSON input (either an absolute path or a path relative to `path`). You can find
