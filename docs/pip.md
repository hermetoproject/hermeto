...
```shell
git clone https://github.com/hermeto/doc-examples --branch=pip-basic
```
...
- "path": "."
- package manager type provided as a CLI arg (pip, gomod, npm, etc.)
...
For our example, we can use the defaults as mentioned: the current directory
(our git repo), and the standard requirements file
...
```shell
podman run --rm localhost/simple_color_output:latest
```
...
[basic pip project]: https://github.com/hermeto/doc-examples/tree/pip-basic
