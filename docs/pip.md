...
If Hermeto detects a Rust-based Python sdist that does not contain Cargo.lock,
it logs a warning and skips Rust prefetch for that package. Missing lock files
are not auto-generated by Hermeto. This applies to both strict and permissive
mode.
...
Cargo.lock **only when the lock exists but is out-of-sync with Cargo.toml**.
This is problematic as well since it makes builds unpredictable in
