...
All project POMs also implicitly inherit from Maven's
[Super POM](https://maven.apache.org/ref/3.9.14/maven-model-builder/super-pom.html), which defines
defaults such as the central repository, standard directory layout, and default plugin versions.
...
<plugin>
  ...
  <!-- Version number should be updated to reflect minimum required features for Hermeto -->
...
In light of this use case, the `maven` generic prefetcher should not be deprecated for the time
being. Documentation will need to guide developers to the right Hermeto package manager for their
use case. This decision should be re-assessed as the `maven` package manager implementation
matures.
...
- **Dynamic dependencies**: Many Maven plugins can download dependencies dynamically, outside of
  the standard `<dependency>` mechanisms in the `pom.xml`. Neither Hermeto nor the lockfile plugin
  can address these situations.
