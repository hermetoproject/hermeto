...
- The `allow_binary` option will be renamed (deprecated) and moved to the `binary` submodel, but will be retained until the next major version for backwards compatibility
...
The combination of the enabled and packages fields within the binary model allows users to select from three distinct fetching strategies. These strategies are analogous to pip's binary-related flags and apply only to package managers where binary fetching is optional (e.g., pip, bundler).

**No Binaries (Analogous to pip install --no-binary)**
Configuration: binary.enabled: false (or the legacy allow_binary: false).
...
Configuration: binary.enabled: true with no packages list defined.

Behavior: Hermeto will attempt to prefetch compatible binaries for all dependencies where possible. In this mode, it will also prefetch the corresponding source distributions to serve as a fallback and maintain a "source-first" posture.

**Only Binaries for Specific Packages (Analogous to pip install --only-binary)**
Configuration: binary.enabled: true and a packages list is provided.
