# Adding Conan Support to Hermeto Should Never Happen
...
decide against supporting it.


## Background

[Conan][] is one of many package managers for C++. At the moment of writing
...
central repository, however it is possible to amend _conanfile.py_
...
<details>
  ...
  ```
  [requires] : a list of rqeuired packages sepcified in `<package-name>/<package-bersion>` format.
     Examples: 
...
`python_requires_exntends` attributes. These attributes accept dependencies in
...
At the moment of writing the first package listed on Conan page [still contains
...
moment of writing a [recommended
...
At the moment of writing none of the recipes in the central repository
...
other thing to  mention about vendoring is that at the moment of writing it is
