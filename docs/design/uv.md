...
`--no-python-downloads` stops uv from downloading an interpreter. uv resolves an interpreter for `requires-python` before it checks anything, and would otherwise fetch a runtime Hermeto never vetted.

uv exits 1 when the check ran and rejected the lockfile - Hermeto fails and asks the user to run `uv lock`. 
It exits 2 when it errored out before checking, most likely because no interpreter in the image satisfies `requires-python`. That is not something the user can act on, so Hermeto warns and continues with the lockfile unverified.
