...
- [Adding uv support to Hermeto](#adding-uv-support-to-hermeto)
  ...
  - [uv support in Hermeto](#uv-support-in-hermeto)
    ...
    - [SBOM](#sbom)
      - [Sources](#sources)
      - [Example purls](#example-purls)
      - [Component properties](#component-properties)
      - [Local sources: path, directory, editable, virtual](#local-sources-path-directory-editable-virtual)
...
> `uv.lock` is a fixed file name. At the time of writing uv offers no way to
> configure it or supply an alternative
> ([astral-sh/uv#6830](https://github.com/astral-sh/uv/issues/6830)). Hermeto therefore relies on the `uv.lock` name and looks for no other.
...
<details>
  ...
  - **`supported-markers =` ["sys_platform == 'linux'", ...]** - *present only if user-declared by* `tool.uv.environments` (limits the set of environments that uv will consider when resolving dependencies).
...
<details>
  <summary><strong>How <code>pyproject.toml</code> declarations map to <code>uv.lock</code> source kinds</strong></summary>
...
## Build process in uv
uv builds packages through the [PEP 517](https://peps.python.org/pep-0517/)
process: [`uv build`](https://docs.astral.sh/uv/concepts/projects/build/#using-uv-build),
the [build frontend](https://packaging.python.org/en/latest/glossary/#term-Build-Frontend),
runs the [build backend](https://packaging.python.org/en/latest/glossary/#term-Build-Backend)
declared in the project's `[build-system]` table (e.g. hatchling, setuptools, or
uv's own [uv_build](https://docs.astral.sh/uv/concepts/build-backend/#using-the-uv-build-backend))
...
The equivalent environment variable is `UV_BUILD_CONSTRAINT`, which accepts a
...
| Source kind                                | Action                                                              |
|--------------------------------------------|---------------------------------------------------------------------|
| `registry`                                 | Download `sdist.url` and/or `wheels[].url`, per the [binary filter](#binary-filters). |
...
When uv installs a package from an sdist, it builds a wheel using the PEP 517 build backend declared in that sdist's `[build-system].requires`. `uv.lock` does not record these dependencies ([astral-sh/uv#5190](https://github.com/astral-sh/uv/issues/5190)).

Hermeto requires build dependencies to be present in a `requirements-build.txt` file in the project directory. If present, entries are downloaded into `{output_dir}/deps/uv/`; if absent, no build-time dependencies are fetched.

During the build, uv finds them there through [`UV_FIND_LINKS`](#uv_find_links).
...
`https://files.pythonhosted.org/...` to `file:///{output_dir}/deps/uv/<filename>`.
...
version = "4.13.0"
source = { registry = "https://pypi.org/simple" }
sdist = { url = "file:///{output_dir}/deps/uv/anyio-4.13.0.tar.gz", hash = "sha256:...", size = 231622, upload-time = "2026-03-24T12:59:09.671Z" }
wheels = [
    { url = "file:///{output_dir}/deps/uv/anyio-4.13.0-py3-none-any.whl", hash = "sha256:...", size = 114353, upload-time = "2026-03-24T12:59:08.246Z" },
]
```
...
name = "flask"
version = "3.2.0.dev0"
source = { path = "{output_dir}/deps/uv/flask-gitcommit-7ef2946f.tar.gz" }
```
...
|--------------------------------|---------------------------|----------------------------------------------|
| `source = { path = "..." }`    | Plain path                | `"{output_dir}/deps/uv/flask-...tar.gz"`             |
| `sdist.url`, `wheels[].url`    | URL with `file://` scheme | `"file:///{output_dir}/deps/uv/anyio-...tar.gz"`     |
...
2. No re-resolution from `pyproject.toml`.
...
One component is emitted for:

- the **main project**, from `pyproject.toml`;
- every **`[[package]]`** in `uv.lock`  except the root project's own entry, dispatched by source kind (see [Sources](#sources));
- every **build-time dependency** not recorded in `uv.lock` (see [2) Build-Time dependencies](#2-build-time-dependencies)).

All components use the `pypi` purl type. Build-time dependencies use the same purl shape as pip's own packages; their properties use uv-specific names - `hermeto:uv:package:build-dependency` and `hermeto:uv:package:binary`.
#### Sources

| `uv.lock` source | Recorded | purl qualifier | Version in lock | Downloaded |
|------------------|----------|----------------|-----------------|------------|
| `registry` (PyPI) | yes | none | always | yes |
| `registry` (other index) | yes | `repository_url=<index>` | always | yes |
| `git` | yes | `vcs_url=git+<url>@<commit>` | always | yes (cloned) |
| `url` | yes | `download_url=<url>` + `checksum=<hash>` | always | yes |
| `path` (local wheel/sdist file) | yes | `vcs_url` + `subpath` | always | no (in tree) |
| `directory` | yes | `vcs_url` + `subpath` | only if static | no (in tree) |
| `editable` | yes | `vcs_url` + `subpath` | only if static | no (in tree) |
| `virtual` | yes | `vcs_url` + `subpath` | only if static | no (in tree) |
| root project | yes | `vcs_url` (from git) + `subpath` (from `--path`, if given) | name/version from `pyproject.toml` | no |


#### Example purls

| `uv.lock` source | Example purl |
|------------------|--------------|
| `registry` (PyPI) | `pkg:pypi/anyio@4.13.0` |
| `registry` (other index) | `pkg:pypi/anyio@4.13.0?repository_url=https://example.com/simple` |
| `git` | `pkg:pypi/flask@3.2.0.dev0?vcs_url=git%2Bhttps://github.com/pallets/flask%40954f5684e4841aad84a8eec7ace7b81a0d3f6831` |
| `url` | `pkg:pypi/httpx@0.28.1?checksum=sha256:...&download_url=https://files.pythonhosted.org/.../httpx-0.28.1.tar.gz` |
| `path` (local wheel/sdist) | `pkg:pypi/example-package@1.0.0?vcs_url=git%2Bhttps://github.com/acme/monorepo%40a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0#dist/example_package-1.0.0-py3-none-any.whl` |
| `directory` | `pkg:pypi/vendored-lib@1.0.0?vcs_url=git%2Bhttps://github.com/acme/monorepo%40a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0#libs/vendored-lib` |
| `editable` | `pkg:pypi/pkg-a@0.1.0?vcs_url=git%2Bhttps://github.com/acme/monorepo%40a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0#packages/pkg-a` |
| `virtual` | `pkg:pypi/dev-tools@0.1.0?vcs_url=git%2Bhttps://github.com/acme/monorepo%40a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0#tools` |
| root project | `pkg:pypi/root-app@0.1.0?vcs_url=git%2Bhttps://github.com/acme/monorepo%40a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0` |
...
|----------|-------|----------|
| `hermeto:found_by` | `hermeto` | always |
| `hermeto:uv:package:build-dependency` | `true` | build-time dependency (not in `uv.lock`; see [2) Build-Time dependencies](#2-build-time-dependencies) |
| `hermeto:uv:package:binary` | `true` | the component is a wheel - only when a [binary filter](#binary-filters) opts the package into wheels |
| `hermeto:missing_hash:in_file` | path (relative to repo root) to the file lacking a hash | a `registry` package in `uv.lock` with no recorded hash, or a build-time dependency in `requirements-build.txt` with no `--hash` |

#### Local sources: path, directory, editable, virtual

- `path`, `directory`, `editable`, and `virtual` all live in the repository tree,
are never fetched, and are recorded identically:
`pkg:pypi/{name}@{version}?vcs_url=git+{origin}@{commit}#{subpath}`

- The root project appears in `uv.lock` as `editable = "."` or `virtual = "."` or `directory = "."`; that
`"."` entry is **skipped** during component generation, because the root is already
emitted from `pyproject.toml`. `path` never use `"."`.

- If the source is not in a git repository, `vcs_url` cannot be resolved. In strict
mode (default) this is an error and the run fails — a local-source purl without
`vcs_url` would be indistinguishable from a registry purl. 
- In permissive mode, `vcs_url` is omitted for every
local-source component derived from repo detection in that run (the root project
and every `path`/`directory`/`editable`/`virtual` dependency).
