...
- **[inject-files](#lockfile-rewriting-inject-files)** - rewrite every artifact URL in `uv.lock` to point into
  `{output_dir}/deps/uv/` instead of the network
...
Hermeto uses the following environment variables for the user's build step:
...
This is a conditional variable which depends upon whether user uses binary filters or not.
