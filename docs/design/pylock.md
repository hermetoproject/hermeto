...
- **Extending the requirements.txt parser**: TOML and line-based formats
  ...
  and it mixes two unrelated parsing responsibilities without a shared
  abstraction to justify it.
- **Separate package manager with dedicated download logic**: introduces
  more code to maintain with no clear benefit over reusing the existing
  pip download pipeline.
...
time, all packages are fetched unconditionally.

> [!NOTE]
> Given the pipeline stage where Hermeto operates and due to the existing binary
> filtering logic environment markers are out of scope.
